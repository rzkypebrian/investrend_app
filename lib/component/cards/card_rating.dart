...
class CardRating extends StatelessWidget {
  ...
  const CardRating(this.notifier,/*this.rating,*/{this.onRetry,Key key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Card(
      margin: EdgeInsets.only(left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral, top: InvestrendTheme.cardPaddingVertical, bottom: InvestrendTheme.cardPaddingVertical),
        ...
        return ValueListenableBuilder<ResearchRank>(
            ...
            builder: (context, value, child) {
              ...
              if(notifier.currentState.isNoData()){
                ...
                subtitle = value.getSubtitle(language: EasyLocalization.of(context).locale.languageCode);
                description = value.getDescription(language: EasyLocalization.of(context).locale.languageCode);
                ranks = value.value;
              }
              ...
              if(notifier.currentState.isError()){
                content = Center(child: TextButtonRetry(onPressed: onRetry,));
              }else{
                content = Text(
