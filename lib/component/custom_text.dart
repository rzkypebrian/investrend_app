...
class FormatTextBullet extends StatelessWidget {
    ...
    lines.forEach((line) {
      if(line.startsWith('• ')){
        ...
        if(StringUtils.isEmtpy(lastBullet)){
          list.add(Text(line,style: style ??= Theme.of(context).textTheme.bodyText2));
        }else{
          list.add(bulletOrLine(context, lastBullet, line, showBulletLine: false));
        }
      }else {
        bool marked = false;
        String marker = '. ';
  ...
  Widget bulletOrLine(BuildContext context, String bulletLine, String text,{bool showBulletLine = true}){
    ...
    return Row(
      ...
      children: [
        SizedBox(
          ...
          child: Text(
            bulletLine,
            style: showBulletLine ? bulletStyle : bulletStyle.copyWith(color: Colors.transparent),
          ),
        ),
        // SizedBox(
        //   width: 5.0,
