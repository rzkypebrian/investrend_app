...
class GridPriceTwo extends StatelessWidget {
  ...
  const GridPriceTwo(this.listData,{this.onSelected, Key key, this.gridCount = 2, this.marginTile= 5.0, this.ratioHeight = 0.0, this.showDecimalPrice = true}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constrains) {
      ...
      double availableWidth = constrains.maxWidth - (marginTile * (gridCount - 1));
      ...
      EdgeInsets padding        = EdgeInsets.only(left: 8.0, right: 8.0, top: 8.0, bottom: 8.0);
      EdgeInsets paddingChange = EdgeInsets.only(left: 8.0, right: 8.0, top: 8.0, bottom: 8.0);
      ...
      */
      double availableWidthForCode    = ((tileWidth - padding.left - padding.right) / 4) * 2;
      double availableWidthForPercent = (((tileWidth - padding.left - padding.right) / 4) * 2) - paddingChange.left - paddingChange.right;
      ...
      for (int i = 0; i < countData; i++) {
        ...
        if(data != null){
          ...
          String priceText = InvestrendTheme.formatPriceDouble(data.price, showDecimal: showDecimalPrice);
          ...
          percentStyle = useFontSize(context, percentStyle, availableWidthForPercent, changeText);
      ...
      if(useRatioHeight){
        ...
        double heightContentRight = UIHelper.textSize('Pj', percentStyle).height;
      ...
      for (int i = 0; i < countData; i++) {
        ...
        if (data != null) {
          ...
          if(rows.length >= gridCount){
            ...
            cols.add(SizedBox(height: marginTile,));
      ...
      if(rows.isNotEmpty){
        ...
        for(int x = 0; x < gap; x++){
          rows.add(SizedBox(width: tileWidth,));
        }
        cols.add(Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: rows,
...
class GridPriceThree extends StatelessWidget {
  ...
  const GridPriceThree(this.listData,{this.stylePrice, this.onSelected,Key key, this.gridCount = 2, this.marginTile= 5.0, this.ratioHeight = 0.0, this.showDecimalPrice = true, this.emptyMessage}) : super(key: key);
    ...
    return LayoutBuilder(builder: (context, constrains) {
      ...
      if(countData == 0) {
        return SizedBox(
          ...
          height: useRatioHeight ? tileHeight : tileWidth,
          child: Center(
            child: EmptyLabel(text: emptyMessage,),
          ),
        );
      }
      ...
      TextStyle codeStyle     = InvestrendTheme.of(context).small_w600_compact;
      TextStyle priceStyle    = stylePrice ?? InvestrendTheme.of(context).support_w600_compact;
      TextStyle percentStyle  = InvestrendTheme.of(context).more_support_w600_compact.copyWith(fontSize: 12.0);
