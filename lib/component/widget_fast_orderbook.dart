...
class WidgetFastOrderBook extends StatefulWidget {
  ...
  const WidgetFastOrderBook(this._orderType, this.maxShowLevel, {Key key, this.owner = '' /*, this.orderDataNotifier, this.calculateNotifier*/
      })
      : super(key: key);
...
class _WidgetFastOrderBookState extends State<WidgetFastOrderBook> {
  ...
  void calculateOrder() {
    ...
    BuySell data = context.read(buySellChangeNotifier).getData(widget._orderType);
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        print('WidgetFastOrderBook [' + widget._orderType.routeName + '] constrains ' + constraints.maxWidth.toString());
  ...
  Widget buildOrderbook(BuildContext context, double widthWidget, double heightWidget) {
    ...
    return Consumer(builder: (context, watch, child) {
      ...
      Widget headers = widget._orderType == OrderType.Buy ? createHeaderBuy(context, widthSection) : createHeaderSell(context, widthSection);
      list.add(headers);

      StockSummary stockSummary = context.read(stockSummaryChangeNotifier).summary;
      int prev = stockSummary != null && stockSummary.prev != null && StringUtils.equalsIgnoreCase(stockSummary.code, notifier.orderbook.code)
          ? stockSummary.prev
          : 0;
      ...
      print('WidgetFastOrderBook [' + widget._orderType.routeName + '] buildOrderbook  code : ' + notifier.orderbook.code + '  prev : $prev');
      ...
      for (int index = 0; index < count; index++) {
        ...
        if (widget._orderType == OrderType.Buy) {
          ...
          list.add(createRowBuy(context, bidQueue, bidLot, bidPrice, bidColor, offerQueue, offerLot, offerPrice, offerColor, widthSection,
              fontSize, _controllers.elementAt(index), showBid, keysBid.elementAt(index), openOrder, focusNode: focusNode, nextFocusNode: nextFocusNode));
        } else {
          ...
          list.add(createRowSell(context, bidQueue, bidLot, bidPrice, bidColor, offerQueue, offerLot, offerPrice, offerColor, widthSection,
              fontSize, _controllers.elementAt(index), showOffer, keysOffer.elementAt(index), openOrder, focusNode: focusNode, nextFocusNode: nextFocusNode));
        }
      }
  ...
  double useFontSize(BuildContext context, double fontSize, double widthSection, OrderBook value, {int offset = 0}) {
    print('WidgetFastOrderBook[' + widget.owner + '].useFontSize Try fontSize  : $fontSize  offset : $offset');
    ...
    for (int index = offset; index < count; index++) {
      ...
      if (widget._orderType == OrderType.Buy) {
        ...
        if (reduceFontSize) {
          fontSize = useFontSize(context, fontSize - font_step, widthSection, value, offset: index);
          //break;
          return fontSize;
        }
      } else {
    ...
    print('WidgetFastOrderBook[' + widget.owner + '].useFontSize Final fontSize  : $fontSize  offset : $offset');
    return fontSize;
  }
  ...
  Widget createHeaderSell(BuildContext context, double widthSection) {
    return Row(
      children: [
        SizedBox(width: widthSection / 3, child: createHeaderRight(context, 'Bid')),
        SizedBox(
        ...
        SizedBox(width: widthSection / 3, child: createHeaderRight(context, 'Open')),
        SizedBox(width: widthSection / 3, child: createHeaderRight(context, 'Lot')),
      ],
    );
  }
  ...
  Widget createHeaderBuy(BuildContext context, double widthSection) {
    return Row(
      children: [
        SizedBox(width: widthSection / 3, child: createHeaderLeft(context, 'Lot')),
        SizedBox(width: widthSection / 3, child: createHeaderLeft(context, 'Open')),
        Container(
          width: widthSection,
          //color: Colors.yellow,
          child: Stack(
        ...
        SizedBox(width: widthSection / 3, child: createHeaderLeft(context, 'Ask')),
      ],
    );
  }

  Widget createRow(BuildContext context, String bidQueue, String bidLot, String bidPrice, Color bidColor, String offerQueue, String offerLot,
      String offerPrice, Color offerColor, double widthSection, double fontSize) {
    return Row(
      children: [
        Container(
          width: widthSection,
        ...
        Container(
          ...
          child: Stack(
            children: [
              createHeaderLeft(context, ''),
              Positioned.fill(child: createLabelPrice(context, fontSize, offerPrice, TextAlign.left, bidColor)),
    ...
    return Row(
      ...
      children: [
        SizedBox(
          ...
          child: createLabelPrice(context, fontSize, bidPrice, TextAlign.right, bidColor),
        ),
        SizedBox(
        ...
        Container(
          ...
          child: createLabelPrice(context, fontSize, '', TextAlign.right, offerColor),
        ),
        SizedBox(
          width: (WidgetFastOrderBook.middle_gap / 2) - 0.5,
        ),
        Container(
          ...
          child: Stack(
            children: [
              ...
              Positioned.fill(child: createLabelPrice(context, fontSize, offerPrice, TextAlign.left, offerColor, key: key)),
              Positioned.fill(child: createLabelLot(context, fontSize, offerLot)),
              Positioned.fill(child: createLabelQueue(context, fontSize, offerQueue, TextAlign.right)),
            ],
          ),
        ...
        SizedBox(
          width: widthSection / 3,
          child: createLabelOpen(context, fontSize, open, TextAlign.right, showOffer),
        ),
        SizedBox(
          width: widthSection / 3,
          child: Padding(
  ...
  Widget createRowBuy(
    ...
    return Row(
      ...
      children: [
        ...
        SizedBox(
          width: widthSection / 3,
          child: createLabelOpen(context, fontSize, open, TextAlign.left, showBid),
        ),
        Container(
          width: widthSection,
          //color: Colors.yellow,
  ...
  Widget createLabelOpen(BuildContext context, double fontSize, int lot, TextAlign textAlign, bool show) {
    return Padding(
      padding: const EdgeInsets.only(top: 5.0, bottom: 5.0),
      child: show
          ? RichText(
              ...
              text: TextSpan(
                ...
                style:
                    InvestrendTheme.of(context).small_w400.copyWith(color: InvestrendTheme.of(context).greyDarkerTextColor, fontSize: fontSize),
                children: <TextSpan>[
                  TextSpan(
                    text: lot > 0 ? '*' : '',
                    style: InvestrendTheme.of(context).small_w400.copyWith(color: Colors.red, fontSize: fontSize),
                  ),
                ],
              ),
            )
  ...
  Widget createLabelPrice(BuildContext context, double fontSize, String text, TextAlign textAlign, Color color, {Key key}) {
    return Padding(
      padding: const EdgeInsets.only(top: 5.0, bottom: 5.0),
      child: Text(
        text,
        key: key,
        style: InvestrendTheme.of(context).small_w500.copyWith(color: color, fontSize: fontSize),
        textAlign: textAlign,
      ),
    );
  }
