// ignore_for_file: require_trailing_commas, must_call_super, unused_local_variable
...
class ScreenMessage extends StatefulWidget {
  ...
  final String caller;
  ScreenMessage({Key? key, this.baseMessage, this.caller = ''})
      : super(key: key);
    ...
    try {
      ...
      if (!StringUtils.isEmtpy(notifType) &&
        ...
        final result = await InvestrendTheme.datafeedHttp
            .reportNotification(notifType, notifId, deviceId, action: 'read');
        if (result != null) {
        ...
        print(routeName +
            ' reportNotification Invalid  notif_type : $notifType  notif_id : $notifId  device_id : $deviceId');
        return 'Invalid';
      }
    } catch (error) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text(
              title!,
              style: InvestrendTheme.of(context).regular_w600?.copyWith(
            ...
            if (!InvestrendTheme.tradingHttp.is_production &&
                widget.baseMessage != null) ...[
              row('caller', caller),
              row('fcm_android_channel_id',
                  widget.baseMessage?.fcm_android_channel_id),
