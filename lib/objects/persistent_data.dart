...
class StoredData {
  ...
  MD5StockBrokerIndex md5 = MD5StockBrokerIndex('', '', '','', 1, '', '', '', '');
  ...
  static Future<StoredData> load() async {
    ...
    if (!StringUtils.equalsIgnoreCase(version, VALID_VERSION)) {
      ...
      print('StoredData.load outdated version $version valid is $VALID_VERSION, cleared : ' + cleared.toString());
    }
    ...
    storedData.unserializeFromString(finderRecentString, storedData.listFinderRecent);
    ...
    MD5StockBrokerIndex md5 = MD5StockBrokerIndex(md5broker, md5stock, md5index, md5sector, sharePerLot, md5brokerUpdate, md5stockUpdate, md5indexUpdate, md5sectorUpdate);
    ...
    //
    print('StoredData.load storedData.listBroker : '+storedData.listBroker.length.toString());
    print('StoredData.load storedData.listStock : '+storedData.listStock.length.toString());
    print('StoredData.load storedData.listIndex : '+storedData.listIndex.length.toString());
    print('StoredData.load storedData.listSector : '+storedData.listSector.length.toString());
    print('StoredData.load storedData.listFinderRecent : '+storedData.listFinderRecent.length.toString());
  ...
  Future<bool> save(/*SharedPreferences prefs*/) async {
    ...
    bool savedBrokerUpdate = await prefs.setString('md5brokerUpdate', md5.md5brokerUpdate);
    bool savedStockUpdate = await prefs.setString('md5stockUpdate', md5.md5stockUpdate);
    bool savedIndexUpdate = await prefs.setString('md5indexUpdate', md5.md5indexUpdate);
    bool savedSectorUpdate = await prefs.setString('md5sectorUpdate', md5.md5sectorUpdate);
    ...
    bool savedBrokerString = await prefs.setString('brokerString', brokerString);
    ...
    bool savedSectorString = await prefs.setString('sectorString', sectorString);
    bool savedFinderRecentString = await prefs.setString('finderRecentString', finderRecentString);
    ...
    if (saved) {
      ...
      print('StoredData.save at : ' + this.updated + "  version : $VALID_VERSION");
    } else {
  ...
  String serializeAsString(List list) {
    ...
    list.forEach((Object object) {
      ...
      if (object is SerializeableSSI) {
        ...
        print('StoredData.serializeAsString is Invalid object!!!! [error] ' + object.toString());
      }
    });
    return string;
  }
