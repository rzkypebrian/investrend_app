  ...
  static String serializeAsString(List list) {
    ...
    list.forEach((Object object) {
      ...
      if (object is Serializeable) {
        ...
        print('serializeAsString is Invalid object!!!! [error] ' + object.toString());
      }
    });
    return string;
  }
...
class Watchlist extends Serializeable{
  ...
  factory Watchlist.fromPlain(String data) {
    List<String> datas = data.split('|');
    if (datas != null && datas.isNotEmpty && datas.length >= 2) {
      String name = Serializeable.unsafePlain(StringUtils.noNullString(datas.elementAt(0)));
      String image = Serializeable.unsafePlain(StringUtils.noNullString(datas.elementAt(1)));
      String stockString = Serializeable.unsafePlain(StringUtils.noNullString(datas.elementAt(2)));
  ...
  static Future<List<Watchlist>> load() async {
    ...
    print('Watchlist.load updated : $updated   watchlistString : $watchlistString');
    Serializeable.unserializeFromString(watchlistString, _listWatchlist);
    print('Watchlist.load Got _listWatchlist : ' + _listWatchlist.length.toString());
  ...
  static Future<bool> save(List<Watchlist> _listWatchlist) async {
    ...
    bool savedWatchlistString = await prefs.setString('watchlist_list', watchlistString);

    bool saved = savedUpdated && savedWatchlistString;
    print('Watchlist.save $saved updated : $updated   savedWatchlistString : $savedWatchlistString  watchlistString : $watchlistString');
    return saved;
  }
