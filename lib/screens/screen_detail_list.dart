...
class ScreenListDetail extends StatefulWidget {
  ...
  const ScreenListDetail(this.list, {this.icon, this.title, this.color, this.indexSector, Key key}) : super(key: key);
...
class _ScreenListDetailState extends VisibilityAwareState<ScreenListDetail> {
  ...
  Future doUpdate({bool pullToRefresh = false}) async {
    ...
    if(widget.indexSector != null){
      try {
        final indexSummarys = await InvestrendTheme.datafeedHttp.fetchIndices([widget.indexSector.code]);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            StringUtils.isEmtpy(widget.icon) ? SizedBox(height: 1.0,) : Image.asset(widget.icon, color: Theme.of(context).primaryColor,),
            SizedBox(width: InvestrendTheme.cardPadding,),
            Text(
              widget.title,
              style: Theme.of(context).textTheme.headline4.copyWith(color: Theme.of(context).primaryColor, fontWeight: FontWeight.w600),
            ),
          ],
        ),
        // actions: [
      ...
      bottomSheet: widget.indexSector != null ? createBottomSheet(context, paddingBottom) : null,
    );
  }
            ...
            return ListView.separated(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  if(index == 0){
                    return Container(
                      color: Theme.of(context).backgroundColor,
                      padding: EdgeInsets.only(left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral, top: InvestrendTheme.cardPaddingGeneral, ),
                      child: Row(
