...
class _ScreenFundOutState extends BaseStateNoTabs<ScreenFundOut> {
  ...
  TextEditingController _priceTextEditingController = TextEditingController(text: '');
  TextEditingController _instructionTextEditingController = TextEditingController(text: '');
            ...
            builder: (context, value, child) {
              return Checkbox(
                  ...
                  onChanged: (value) {
                    ...
                    print('_agreeTnCNotifier '+_agreeTnCNotifier.value.toString());
                  });
            },
          ),
          Flexible(
            child: Text(
                'fund_out_agreement_content'.tr(), style: InvestrendTheme.of(context).support_w400),
  ...
  void didChangeDependencies() {
    ...
    if(onAccountChangeListener != null){
      context.read(accountChangeNotifier).removeListener(onAccountChangeListener);
    }else{
  ...
  void dispose() {
    ...
    if(onAccountChangeListener != null){
      container.read(accountChangeNotifier).removeListener(onAccountChangeListener);
    }
  ...
  Future doUpdate({bool pullToRefresh = false}) async {
    ...
    try {
      ...
      final result = await InvestrendTheme.tradingHttp
          .getBankRDN(active.accountcode, InvestrendTheme.of(context).applicationPlatform, InvestrendTheme.of(context).applicationVersion);
      if (result != null) {
    ...
    try {
      ...
      final result = await InvestrendTheme.tradingHttp
          .getBankAcccount(active.accountcode, InvestrendTheme.of(context).applicationPlatform, InvestrendTheme.of(context).applicationVersion);
      if (result != null) {
    ...
    try {
      ...
      final result = await InvestrendTheme.tradingHttp.cashPosition(active.brokercode, active.accountcode, user.username,
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, BankRDN value, child) {
        Widget mainWidget = Padding(
          padding: const EdgeInsets.only(left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral, top: 12.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(
        ...
        Widget noWidget = _bankRDNNotifier.currentState.getNoWidget(onRetry: () => doUpdate(pullToRefresh: true));
        if (noWidget != null) {
          return Stack(
            children: [
              mainWidget,
      ...
      builder: (context, final BankAccount value, child) {
        ...
        Widget mainWidget = Column(
          ...
          children: [
            ...
            //   height: 4.0,
            // ),
            TextButtonDropdown(_bankDestinationNotifier, list,clickAndClose: true, style: InvestrendTheme.of(context).small_w400_compact,),
            /*
            Text(
              value.bank + ' - ' + value.acc_no,
              style: InvestrendTheme.of(context).small_w400,
            ...
            */
            ...
            Padding(
              padding: const EdgeInsets.only(left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral),
              child: ComponentCreator.textFieldForm(context, lightTheme, 'Rp ', '', null, null, '', false, TextInputType.number,
                  TextInputAction.done, null, _priceTextEditingController, null, _focusNodePrice, null,
                  padding: EdgeInsets.only(top: 1.0), inputFormatters: [
                  PriceFormatter(),
                ],),
            ),
            /*
                  ...
                  ValueListenableBuilder(
                    ...
                    builder: (context, CashPosition value, child) {
                      ...
                      return Text(
                        ...
                        style: InvestrendTheme.of(context).small_w400.copyWith(color: Theme.of(context).accentColor),
                      );
                    },
                  ),
                ],
              ...
              builder: (context,  value, child) {
                ...
                return TextButton(
                  style: TextButton.styleFrom(
                      padding: EdgeInsets.only(left:InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral, top: InvestrendTheme.cardPadding, bottom: InvestrendTheme.cardPadding),
                      alignment: Alignment.centerLeft),
                  child: Text(value, style: Theme.of(context).textTheme.button.copyWith(color: InvestrendTheme.of(context).investrendPurple),),
            ...
            Padding(
              padding: const EdgeInsets.only(left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral),
              child: ComponentCreator.textFieldForm(context, lightTheme, '', '', null, null, '', false, TextInputType.multiline,
                TextInputAction.done, null, _instructionTextEditingController, null, _focusNodeInstruction, null,
                padding: EdgeInsets.only(top: 1.0) ,),
            ),
          ],
        );
        Widget noWidget = _bankAccountNotifier.currentState.getNoWidget(onRetry: () => doUpdate(pullToRefresh: true));
        if (noWidget != null) {
          return Stack(
            children: [
              mainWidget,
  ...
  void submitFundOut(BuildContext context) async{
    ...
    if( !hasAccount ){
      InvestrendTheme.of(context).showSnackBar(context, 'no_account_found_message'.tr());
      return;
    }
    User user = context.read(dataHolderChangeNotifier).user;
    Account active = user.getAccount(notifier.index);
    if (active == null) {
      InvestrendTheme.of(context).showSnackBar(context, 'no_active_account_found_message'.tr());
      return;
    }
    if(_bankRDNNotifier.value.isEmpty()){
      InvestrendTheme.of(context).showSnackBar(context, 'no_source_bank_found_message'.tr());
      return;
    }
    if(_bankAccountNotifier.value.isEmpty()){
      InvestrendTheme.of(context).showSnackBar(context, 'no_destination_bank_found_message'.tr());
      return;
    }
    String scrappedAmount = _priceTextEditingController.text.replaceAll(',', '');
    //int amountNumber = Utils.safeInt(_textEditingController.text);
    int amountNumber = Utils.safeInt(scrappedAmount);
    if(amountNumber <= 0){
      InvestrendTheme.of(context).showSnackBar(context, 'error_amount_is_empty'.tr());
      return;
    }
    ...
    String bank = firstBank ? _bankAccountNotifier.value.bank : _bankAccountNotifier.value.bank2;
    String bankno = firstBank ? _bankAccountNotifier.value.acc_no : _bankAccountNotifier.value.acc_no2;
    String bankname = firstBank ? _bankAccountNotifier.value.acc_name : _bankAccountNotifier.value.acc_name2;
    ...
    try{
      final result = await InvestrendTheme.tradingHttp.submitFundOut(email, realname, account, amount, rdnbank, rdnno, rdnname, bank, bankno, bankname, date, message,platform, version);
      ...
      if(result is String){
        ...
        if(StringUtils.equalsIgnoreCase(result, 'sent')){
          ...
          InvestrendTheme.of(context).showInfoDialog(context, title: 'info_label'.tr(), content: 'Withdrawal Instruction has been sent.'.tr(), onClose: (){
  ...
  Widget createBody(BuildContext context, double paddingBottom) {
    return ComponentCreator.keyboardHider(context, RefreshIndicator(
      ...
      child: ListView(
        children: [
          ...
          Padding(
            padding: const EdgeInsets.only(
                top: 5.0, bottom: 5.0, left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Expanded(flex: 1, child: ComponentCreator.divider(context, thickness: 1.0)),
                Image.asset(
          ...
          Padding(
            padding: const EdgeInsets.only(
                bottom: InvestrendTheme.cardPaddingGeneral, left: InvestrendTheme.cardPaddingGeneral, right: InvestrendTheme.cardPaddingGeneral),
            child: Text(
