    ...
    extends State<ScreenSplash> // with SingleTickerProviderStateMixin
{
  ...
  */
  ...
  void initState() {
    ...
    myDevice.load().then((value) {
      myDevice.createUniqueIDIfPossible().whenComplete(() {
        print(routeName +
            ' richy_20220708 myDevice.unique_id = ' +
            myDevice.unique_id);
      });
    });
  ...
  void didChangeDependencies() {
    ...
    DebugWriter.info(
        routeName + ' didChangeDependencies firstTime : $firstTime');
    //EasyLocalization.of(context).setLocale(Locale('id'));
    //TextStyle tst = InvestrendTheme.of(context).regular_w500;
    DebugWriter.info(routeName +
        ' didChangeDependencies datafeedHttp.isLoaded : ' +
    ...
    if (firstTime) {
      ...
      var result = context
          ...
          .properties
          .load(caller: 'ScreenSplash');
      result.then((value) {
        //String languageCode = context.read(propertiesNotifier).properties.getString('/settings', 'app_language', 'id');
        //EasyLocalization.of(context).setLocale(Locale('id'));
        //EasyLocalization.of(context).setLocale(Locale(languageCode));
        if (value) {
          context.read(propertiesNotifier).properties.save().whenComplete(() {
            //Timer(new Duration(milliseconds: 1000), showNextPage);

            String languageCode = context
                .read(propertiesNotifier)
                .properties
                .getString('/settings', 'app_language', 'id');
            DebugWriter.info(routeName +
                ' richy_20220708 didChangeDependencies A | properties.languageCode = [' +
                languageCode +
                ']');
          ...
          String languageCode = context
              ...
              .getString('/settings', 'app_language', 'id');
          DebugWriter.info(routeName +
              ' richy_20220708 didChangeDependencies B | properties.languageCode = [' +
              languageCode +
              ']');
          EasyLocalization.of(context)
              .setLocale(Locale(languageCode))
      ...
      children: [
        ...
        AnimatedOpacity(
          ...
          onEnd: () {
            if (animate) {
              /*
              var result = context.read(propertiesNotifier).properties.load();
              result.then((value) {
                //String languageCode = context.read(propertiesNotifier).properties.getString('/settings', 'app_language', 'id');
                //EasyLocalization.of(context).setLocale(Locale('id'));
                //EasyLocalization.of(context).setLocale(Locale(languageCode));
