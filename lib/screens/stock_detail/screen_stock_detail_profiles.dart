...
class ScreenStockDetailProfiles extends StatefulWidget {
  ...
  ScreenStockDetailProfiles(this.tabIndex, this.tabController,
      {Key? key, this.visibilityNotifier})
      : super(key: key);

  @override
  _ScreenStockDetailProfilesState createState() =>
      _ScreenStockDetailProfilesState(tabIndex, tabController!,
          visibilityNotifier: visibilityNotifier!);
}

class _ScreenStockDetailProfilesState
    extends BaseStateNoTabsWithParentTab<ScreenStockDetailProfiles> {
  //CompanyProfileNotifier _companyProfileNotifier = CompanyProfileNotifier(DataCompanyProfile.createBasic());
  LabelValueNotifier _historyNotifier =
      LabelValueNotifier(new LabelValueData());
  LabelValueNotifier _shareHolderCompositionNotifier =
      LabelValueNotifier(new LabelValueData());
  LabelValueNotifier _boardOfCommisionersNotifier =
      LabelValueNotifier(new LabelValueData());

  _ScreenStockDetailProfilesState(int tabIndex, TabController tabController,
      {ValueNotifier<bool>? visibilityNotifier})
      : super('/stock_detail_profiles', tabIndex, tabController,
  ...
  Future doUpdate({bool pullToRefresh = false}) async {
    ...
    if (!StringUtils.isEmtpy(code)) {
      ...
      try {
        final DataCompanyProfile? result =
            await InvestrendTheme.datafeedHttp.fetchCompanyProfile(code);
        if (result != null && !result.isEmpty()!) {
          if (mounted) {
            ...
            dataHistory.datas?.add(LabelValue(
                'card_history_listing_date_label'.tr(), result.listing_date));
            dataHistory.datas?.add(LabelValue(
                'card_history_effective_date_label'.tr(),
                result.effective_date));
            dataHistory.datas?.add(
                LabelValue('card_history_nominal_label'.tr(), result.nominal));
            dataHistory.datas?.add(LabelValue(
                'card_history_ipo_price_label'.tr(), result.ipo_price));
            dataHistory.datas?.add(LabelValue(
                'card_history_ipo_shares_label'.tr(), result.ipo_shares));
            dataHistory.datas?.add(LabelValue(
                'card_history_ipo_amount_label'.tr(), result.ipo_amount));
            dataHistory.datas?.add(LabelValueDivider());

            int? countUnderwriter = result.countUnderwriter();
            if (countUnderwriter == 0) {
              dataHistory.datas
                  ?.add(LabelValue('card_history_underwriter_label'.tr(), '-'));
            } else {
              for (int i = 0; i < countUnderwriter!; i++) {
                if (i == 0) {
                  dataHistory.datas?.add(LabelValue(
                      'card_history_underwriter_label'.tr(),
                      result.underwriter_list?.elementAt(i)));
                } else {
                  dataHistory.datas?.add(
                      LabelValue(' ', result.underwriter_list?.elementAt(i)));
                }
              }
            }
            ...
            if (countShareRegistrar == 0) {
              dataHistory.datas?.add(
                  LabelValue('card_history_share_registrar_label'.tr(), '-'));
            } else {
              for (int i = 0; i < countShareRegistrar!; i++) {
                if (i == 0) {
                  dataHistory.datas?.add(LabelValue(
                      'card_history_share_registrar_label'.tr(),
                      result.share_registrar_list?.elementAt(i)));
                } else {
                  dataHistory.datas?.add(LabelValue(
                      ' ', result.share_registrar_list?.elementAt(i)));
                }
            ...
            for (int i = 0; i < countContentList!; i++) {
              ...
              if (dc!.isDivider()) {
                ...
                dataShareholders.datas?.add(LabelValuePercent(
                    dc.text_1, dc.text_2, dc.text_3,
                    valuePercentColor: dc.color));
              }
            }
            ...
            addTo(
                dataCommisioners,
                result.countPresidentCommissioner(),
                'card_board_of_commisioners_president_commissioner_label'.tr(),
                result.president_commissioner_list);
            addTo(
                dataCommisioners,
                result.countVicePresidentCommissioner(),
                'card_board_of_commisioners_vice_president_commissioner_label'
                    .tr(),
                result.vice_president_commissioner_list);
            addTo(
                dataCommisioners,
                result.countCommissioner(),
                'card_board_of_commisioners_commissioner_label'.tr(),
                result.commissioner_list);
            dataCommisioners.datas?.add(LabelValueDivider());

            addTo(
                dataCommisioners,
                result.countPresidentDirector(),
                'card_board_of_commisioners_president_director_label'.tr(),
                result.president_director_list);
            addTo(
                dataCommisioners,
                result.countVicePresidentDirector(),
                'card_board_of_commisioners_vice_president_director_label'.tr(),
                result.vice_president_director_list);
            addTo(
                dataCommisioners,
                result.countDirectorList(),
                'card_board_of_commisioners_director_label'.tr(),
                result.director_list);
  ...
  void addTo(
      LabelValueData data, int? count, String label, List<String>? content) {
    //int countPresidentCommissioner = result.countPresidentCommissioner();
    if (count == 0) {
  ...
  Widget createBody(BuildContext context, double paddingBottom) {
    List<Widget> childs = [
      ...
      CardLabelValueNotifier('card_shareholders_composition_title'.tr(),
          _shareHolderCompositionNotifier),
      ComponentCreator.divider(context),
      CardLabelValueNotifier('card_board_of_commisioners_title'.tr(),
          _boardOfCommisionersNotifier),
      SizedBox(
