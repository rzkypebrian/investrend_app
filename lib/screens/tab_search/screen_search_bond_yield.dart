...
class ScreenSearchBondYield extends StatefulWidget {
  ...
  ScreenSearchBondYield(this.tabIndex, this.tabController,
      {Key? key, this.visibilityNotifier})
      : super(key: key);

  @override
  _ScreenSearchBondYieldState createState() =>
      _ScreenSearchBondYieldState(tabIndex, tabController,
          visibilityNotifier: visibilityNotifier!);
}

class _ScreenSearchBondYieldState
    extends BaseStateNoTabsWithParentTab<ScreenSearchBondYield> {
  ...
  _ScreenSearchBondYieldState(int tabIndex, TabController? tabController,
  ...
  Future doUpdate({bool pullToRefresh = false}) async {
    ...
    try {
      final GroupedData? groupedData =
          await InvestrendTheme.datafeedHttp.fetchBonds();
      if (groupedData != null) {
        if (mounted) {
