...
class ScreenAmendBuy extends StatefulWidget {
  ...
  const ScreenAmendBuy(this.amendData,this.updateDataNotifier, {Key key, this.keyboardNotifier}) : super(key: key);

  @override
  _ScreenAmendBuyState createState() => _ScreenAmendBuyState(amendData.orderType, amendData,updateDataNotifier, keyboardNotifier: keyboardNotifier);
}
class _ScreenAmendBuyState extends BaseAmendState<ScreenAmendBuy>
    //with AutomaticKeepAliveClientMixin<ScreenAmendBuy>
{
  _ScreenAmendBuyState(OrderType orderType, BuySell amendData,ValueNotifier<bool> updateDataNotifier,{ValueNotifier<bool> keyboardNotifier}) : super(orderType , amendData,updateDataNotifier, keyboardNotifier: keyboardNotifier);

  StringColorFontNotifier notifierBuyingPower = StringColorFontNotifier(StringColorFont());
  StringColorFontBoolNotifier notifierCashAvailable = StringColorFontBoolNotifier(StringColorFontBool());
  //StringColorFontBoolNotifier notifierCashBalance = StringColorFontBoolNotifier(StringColorFontBool());
  StringColorFontBoolNotifier notifierBuyingPowerNew = StringColorFontBoolNotifier(StringColorFontBool());
  ...
  void dispose() {
    ...
    if (onEventBuyingPowerCashAvailable != null) {
      container.read(buyRdnBuyingPowerChangeNotifier).removeListener(onEventBuyingPowerCashAvailable);
    }
  ...
  void didChangeDependencies() {
    ...
    if (onEventBuyingPowerCashAvailable != null) {
      context.read(buyRdnBuyingPowerChangeNotifier).removeListener(onEventBuyingPowerCashAvailable);
    } else {
      onEventBuyingPowerCashAvailable = () {
        if (mounted) {
          double widthAvailable =
              MediaQuery.of(context).size.width - InvestrendTheme.cardPaddingGeneral - InvestrendTheme.cardPaddingGeneral - 10.0 - 10.0;
          print(orderType.routeName + ' onEventBuyingPowerCashBalance .widthAvailable : $widthAvailable');
          double widthValue = widthAvailable * 0.47;

          double buyingPower = context.read(buyRdnBuyingPowerChangeNotifier).buyingPower;
          double cashBalance = context.read(buyRdnBuyingPowerChangeNotifier).cashAvailable;
          double creditLimit = context.read(buyRdnBuyingPowerChangeNotifier).creditLimit;

          String buyingPowerText = InvestrendTheme.formatMoneyDouble(buyingPower, prefixRp: true);
          String cashBalanceText = InvestrendTheme.formatMoneyDouble(cashBalance, prefixRp: true);

          TextStyle style = InvestrendTheme.of(context).regular_w600;
          style = UIHelper.useFontSize(context, style, widthValue, buyingPowerText);
          style = UIHelper.useFontSize(context, style, widthValue, cashBalanceText);
          /*
          ...
          */
          ...
          notifierBuyingPowerNew.setValue(buyingPowerText, fontSize: style.fontSize, newColor: style.color, boolFlag: activeBuyingPower);
          notifierCashAvailable.setValue(cashBalanceText, fontSize: style.fontSize, newColor: style.color, boolFlag: activeCashAvailable);

        }
      };
    }
    context.read(buyRdnBuyingPowerChangeNotifier).addListener(onEventBuyingPowerCashAvailable);
  }
          ...
          ValueListenableBuilder(
            ...
            builder: (context, StringColorFontBool value, child) {
              ...
              return ButtonInfo('trade_buy_label_buying_power'.tr(), value, (){
                ...
                predefineLotSourceNotifier.value = 'trade_buy_label_buying_power'.tr();
              });
            },
          ),
          ValueListenableBuilder(
            ...
            builder: (context, StringColorFontBool value, child) {
              return ButtonInfo('cash_available_label'.tr(), value, (){
  ...
  Future doUpdateAccount({bool pullToRefresh = false}) async {
    if (!isActiveAndMounted()) {
      print(orderType.routeName + '.doUpdateAccount Aborted : ' + DateTime.now().toString() + "  _active : $active  mounted : $mounted pullToRefresh : $pullToRefresh");
      return false;
    }
    print(orderType.routeName + '.doUpdateAccount : ' + DateTime.now().toString() + "  _active : $active  mounted : $mounted  pullToRefresh : $pullToRefresh");
    ...
    if (account == null) {
      ...
      accountStockPosition.then((value) {
        DebugWriter.information(orderType.routeName+' Got accountStockPosition  accountStockPosition.size : ' + value.length.toString());
        if(!mounted){
        ...
        DebugWriter.information(orderType.routeName+' accountStockPosition Exception : ' + e.toString());
        if(!mounted){
          onProgressAccount = false;
          return false;
        }
