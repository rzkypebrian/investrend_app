...
class MarketColors {
  ...
  static Color priceColor(num change) =>
      change == 0 ? _noChange : (change < 0 ? _down : _up);

  static Color priceColorTheme(bool dark, num change) => change == 0
      ? (dark ? _noChangeDark : _noChange)
      : (change < 0 ? (dark ? _downDark : _down) : (dark ? _upDark : _up));

  //MediaQuery.of(context).platformBrightness == Brightness.light
  static Color bracketColor(num change) =>
      change == 0 ? _noChangeBracket : (change < 0 ? _downBracket : _upBracket);
  static Color bracketColorTheme(bool dark, num change) => change == 0
      ? (dark ? _noChangeBracketDark : _noChangeBracket)
      : (change < 0
          ? (dark ? _downBracketDark : _downBracket)
          : (dark ? _upBracketDark : _upBracket));
